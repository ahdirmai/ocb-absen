...
> **Radius lokasi absen (`checkAbsensi` middleware):** GPS wajib aktif (koordinat blank/invalid → HTTP 400). Absen (masuk & keluar, regular & lembur) hanya diterima bila **dalam radius `retail_id` yang dikirim**. Di luar radius → **HTTP 400 langsung ditolak** — tak ada fallback ke OC terdekat, tak ada jalur "menunggu approval". Untuk lembur hal ini disengaja: karyawan sudah memilih OC tempat lembur, jadi radius dicek terhadap OC itu. Retail tanpa data `latitude`/`longitude`/`radius` (blank) dilewati (tak dicek). Sebelumnya luar radius tapi dekat OC lain lolos dengan `is_approval=1`; jalur itu (`findNearbyRetail`) dibuang.
