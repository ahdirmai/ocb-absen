...
### Added
- **UI/UX baru halaman Config Menu User (toggle Lama/Baru)** (`MenuCategory.jsx`) — pola sama Absensi (persist `localStorage` key `menucategory_ui_mode`, default Baru; UI lama utuh). Stat cards (Total Config/Kategori User/Menu Unik), search global lintas kolom, kolom sortable (Kategori User ikon grup, Nama Menu badge, Parent Menu, default sort A→Z), aksi ikon. Modal Tambah & Edit didesain ulang (`menuPosition: fixed` cegah dropdown kepotong). Buang 2 `console.log` di render + dead state `search`. FE only.
